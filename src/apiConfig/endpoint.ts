...
export const GET_ALL_PATHWAY_COMPONENT =
  `${process.env.REACT_APP_API_URL}`.includes('finderApi')
...
export const GET_DATA_FOR_PATHWAY = `${process.env.REACT_APP_API_URL}`.includes(
  'finderApi'
)
  ? 'PathwayDisplay/'
...
export const FINDER_URL = `${process.env.REACT_APP_API_URL}`.includes('sandbox')
